...
async fn perform_completion(
    ...
    let stream = match params.provider {
        ...
        LanguageModelProvider::AzureOpenAi => {
            let api_key = state
                .config
                .openai_api_key
                .as_ref()
                .context("no Azure OpenAI API key configured on the server")?;
            let chunks = azure_open_ai::stream_completion(
                &state.http_client,
                azure_open_ai::AZURE_OPEN_AI_API_URL,
                api_key,
                serde_json::from_str(&params.provider_request.get())?,
                None,
            )
